...
from backend.core.csrf import enforce_csrf  # noqa: E402
...
def safe_web_file(relative_path: str) -> Path | None:
...
async def serve_spa(full_path: str):
    ...
    这样刷新页面时不会 404
    """
    file_path = safe_web_file(full_path)
    if file_path:
        return FileResponse(file_path)

    html_path = safe_web_file(f"{full_path}.html")
    if html_path:
