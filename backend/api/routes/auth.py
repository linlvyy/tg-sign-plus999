...
from backend.core.csrf import clear_csrf_cookie, set_csrf_cookie
...
def reset_totp(request: Request, payload: ResetTOTPRequest, db: Session = Depends(get_db)):
    if not settings.allow_password_totp_reset:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Password-only TOTP reset is disabled",
        )
    ...
    log_audit(
        ...
        user_agent=request.headers.get("User-Agent"),
        status="success",
        details={"method": "password_emergency"},
    )
