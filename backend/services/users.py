...
def ensure_admin(db: Session, username: str = "admin", password: str = None):
    ...
    if not password:
        ...
        if env_pwd:
            ...
            password_file = get_settings().resolve_base_dir() / "initial_admin_password.txt"
            password_file.write_text(password, encoding="utf-8")
            password_file.chmod(0o600)
            logger.warning(
                "SECURITY WARNING: Default admin account created with a random password. "
                f"Password file: {password_file}. Change it immediately or set ADMIN_PASSWORD."
            )
